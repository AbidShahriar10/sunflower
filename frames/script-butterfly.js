...
const resizeCamera = () => {
  const x =
    Math.min(
      resolution.x / resolution.y / (CAMERA_SIZE_X / CAMERA_SIZE_Y),
      1.0
    ) * CAMERA_SIZE_X;
  const y =
    Math.min(
      resolution.y / resolution.x / (CAMERA_SIZE_Y / CAMERA_SIZE_X),
      1.0
    ) * CAMERA_SIZE_Y;
  camera.left = x * -0.5;
  camera.right = x * 0.5;
...
const init = () => {
  ...
  renderer.setClearColor(0xe6dcdc, 1.0);
  camera.position.set(250, 500, 1000);
  camera.lookAt(new THREE.Vector3());

  loader.crossOrigin = "anonymous";
  loader.load(
    "http://ykob.github.io/sketch-threejs/img/sketch/butterfly/tex.png",
    (texture) => {
